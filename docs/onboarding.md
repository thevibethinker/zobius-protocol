...
# David Onboarding (No GitHub Login)

This setup keeps the repo public and requires no GitHub auth for David.

## 1) V (host) setup

In Zo Settings > Advanced, add a per-partner secret:

`ZO2ZO_TOKEN_DAVID=<strong-random-token>`

The bridge endpoint is:

`https://va.zo.space/api/bridge`

## 2) David install command

David runs this once in his Zo terminal:
...
David adds these in his Zo Settings > Advanced:
